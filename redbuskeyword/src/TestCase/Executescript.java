package TestCase;

import java.util.Properties;

import org.apache.poi.ss.usermodel.Row;
import org.apache.poi.ss.usermodel.Sheet;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.firefox.FirefoxDriver;
import org.testng.annotations.Test;

import ExcelRead.ReadExcelFile;
import Operation.ReadObject;
import Operation.UIoperation;

public class Executescript
{
	@Test
	public void testLogin() throws Exception
	{
		// TODO Auto-generated method stub
		System.setProperty("webdriver.gecko.driver", "C:\\Users\\Administrator\\Downloads\\geckodriver-v0.18.0-win64\\geckodriver.exe");
		WebDriver webdriver = new FirefoxDriver();
		ReadExcelFile file = new ReadExcelFile();
		ReadObject object = new ReadObject();
		Properties allObjects = object.getObjectRepository();
		UIoperation operation = new UIoperation(webdriver);
		//Read keyword sheet
		Sheet redbusSheet = file.readExcel(System.getProperty("user.dir") + "\\src\\DataEngine", "Redbus.xlsx", "Sheet1");
		//Find number of rows in excel file
		int rowCount = redbusSheet.getLastRowNum() - redbusSheet.getFirstRowNum();

		System.out.println(rowCount);

		//Create a loop over all the rows of excel file to read it
		for (int i = 1; i < (rowCount + 1); i++)
		{
			//Loop over all the rows
			Row row = redbusSheet.getRow(i);

			//Check if the first cell contain a value, if yes, That means it is the new testcase name
			if ((row.getCell(0) != null))
			{
				//Print the new testcase name when it started
				System.out.println("New Testcase->" + row.getCell(0).toString() + " Started");
			}
			else
			{

				//Print testcase detail on console
				System.out.println((row.getCell(1) + "").toString() + "----" + (row.getCell(2) + "").toString() + "----" + (row.getCell(3) + "").toString() + "----" + (row.getCell(4) + "").toString());
				//Call perform function to perform operation on UI
				operation.perform(allObjects, row.getCell(1).toString(), row.getCell(2).toString(), row.getCell(3).toString(), row.getCell(4).toString());
			}
		}
	}

}
